...
for i in range(len(tissue_identifiers)):
    indicesToKeep = df['lab_num'] == tissue_identifiers[i]
    indices_gtex = df['sample'].str.contains('GTEX')
    indices_tcga = ~df['sample'].str.contains('GTEX')

    ax.scatter(df.loc[indicesToKeep*indices_gtex, 'PC1']
        , df.loc[indicesToKeep*indices_gtex, 'PC2']
        , color = colors[i]
        , s = 20, label = 'GTEx {}'.format(targets[i]))

    ax.scatter(df.loc[indicesToKeep*indices_tcga, 'PC1']
        , df.loc[indicesToKeep*indices_tcga, 'PC2']
        , color = colors[i]
